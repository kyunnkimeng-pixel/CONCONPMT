...
| F011 | Icon tile label/edit action, planned icon rename action | R2 | Rename can be inline or through implemented context action. |
| F012 | Editor metadata area and generated thumbnail/cover preview handling | R3 | Only expose override controls when implemented. |
...
| F024 | Planned collection context action and clone collection command | R1 | Not shown in generated image; still required by inventory. |
...
| F035 | Collection settings/editor size controls | R3, R4 | Custom sizes must be possible without weakening DCInside defaults. |
...
| F053 | Main `+` menu action: `폴더 가져오기` | R1 | Folder import requires native dialog/import command later. |
...
| F059 | App startup route/view restore via `app_settings` | R1, R2 | No visible menu required; restore behavior is observable on restart. |
| F060 | Delete confirmation, soft-delete repository, explicit cleanup command/dialog | R2 | Do not add cleanup UI until the explicit destructive flow is implemented. |
| F061 | Conditional context actions: reveal original / reveal export result | R2 | Not shown in generated context menu, but still required where applicable. |
| F062 | Collection cover image import/edit flow for 200x200 JPG/PNG | R1, R2 | Can be part of cover edit dialog or implemented context action. |
