...
| F067 | Editor panel live draft preview and resizable width | done | `EditorPanel.tsx` + `EditorOutputPreview.tsx` keep a compact live output preview directly below the crop canvas and sticky above scrolled settings; persisted resize handle and fixed-height route layout remain | `EditorOutputPreview.test.tsx` / `npm.cmd run lint` / `npm.cmd run build` |
