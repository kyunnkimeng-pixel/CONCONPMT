...
| F103 | Static sprite sheet import | done | `sheet/importer.rs`, `SheetImportWizard.tsx`, `SheetGridOverlay.tsx`, `SheetCellReviewGrid.tsx` | `cargo test` / `npm.cmd run test` |
| F104 | Static edit sheet export | done | `sheet/exporter.rs`, `SheetExportDialog.tsx`, `SheetExportPreview.tsx` | `cargo test` / `npm.cmd run test` |
| F105 | Clean sheet + guide sheet split output | done | `sheet/exporter.rs` writes separate `clean/` and `guide/` PNG pages; clean sheet has no grid/labels | `sheet::exporter::tests::edit_sheet_export_writes_clean_guide_and_manifest_with_alpha` |
| F106 | `pmtcon-sheet-v1` manifest generation | done | `sheet/manifest.rs` + `export_edit_sheet` | `sheet::manifest::tests::static_manifest_schema_accepts_required_shape` |
| F107 | Manifest-based static sheet reimport | done | `sheet/reimport.rs`, `SheetReimportDialog.tsx` creates new icons or processed variant files without overwriting originals | `sheet::reimport::tests::reimport_maps_manifest_cell_to_new_icon_without_overwriting_original` |
| F108 | PNG alpha preservation for sheet import/export/reimport | done | `sheet/importer.rs`, `sheet/exporter.rs`, `sheet/reimport.rs` keep RGBA PNG cells and transparent clean sheet backgrounds | `sheet::importer::tests::imported_png_sheet_cells_preserve_alpha_and_original_sheet` / `sheet::exporter::tests::edit_sheet_export_writes_clean_guide_and_manifest_with_alpha` |
| F109 | GIF first-frame contact sheet mode | doing | `sheet/exporter.rs` renders GIF first frame into static work sheets and returns warnings; dedicated UI copy labels static contact-sheet behavior | `cargo test`; dedicated GIF contact-sheet regression pending |
| F110 | GIF frame sheet export | future | `sheet/gif_frames.rs` DTOs and `pmtcon-gif-frame-sheet-v1` manifest planning; no visible action wired | `sheet::gif_frames::tests::gif_frame_manifest_preserves_duration_loop_and_page_mapping` |
| F111 | GIF frame sheet reimport | future | `docs/SPRITE_SHEET_TOOLS_DESIGN.md`, `docs/SPRITE_SHEET_MANIFEST_SPEC.md`, `sheet/gif_frames.rs` result DTO | design only |
| F112 | Manual slice mode | future | `docs/SPRITE_SHEET_TOOLS_DESIGN.md`, `sheet/slices.rs`, `ManualSliceCanvas.tsx` future component not exposed as active UI | design/model only |
| F113 | Auto-detect sheet slicing future | future | `docs/SPRITE_SHEET_TOOLS_DESIGN.md`, `docs/SPRITE_SHEET_WORKFLOW.md` | design only |
