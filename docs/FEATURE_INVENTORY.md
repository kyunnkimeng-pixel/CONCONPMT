...
| F011 | 아이콘명 rename | todo | final review: no implemented icon display-name rename UI/command found | UI test |
| F012 | 아이콘별 대표 썸네일 override | todo | final review: no implemented icon-specific thumbnail/cover override found | unit/manual |
...
| F022 | 우클릭 메뉴: delete/duplicate/edit/set cover | done | `IconContextMenu.tsx`; rename remains F011 | `npm.cmd run lint` |
| F023 | 아이콘 복제 | done | `duplicate_icon` command clones source/crop data and assigns unique alt | `cargo test` |
| F024 | 디시콘 모음 복제 | todo | `duplicate_collection` backend command exists; final review: UI action pending | Rust test |
...
| F035 | collection/icon 기준 사이즈 변경 | todo | editor icon overrides use collection defaults; final review: collection settings UI pending | `npm.cmd run lint` / `crop-math.test.ts` |
...
| F048 | transparent background / 5px margin warnings | todo | export validator warns for JPG transparency risk and upscaling/padding; final review: 5px margin heuristic pending | `cargo test` |
...
| F051 | no generated UI-only dead menus | done | implemented import actions; folder import remains disabled with 준비 중 text; final review removed unused browser/localStorage scaffold mock | review + source search |
| F052 | UI image generation reference workflow | done | `docs/UI_IMAGE_PROMPT.md` + `docs/UI_TRACE.md` + `docs/ui-references/` | manual |
| F053 | 상단 `+` 메뉴: 폴더 가져오기 | todo | disabled 준비 중 control; folder traversal command pending | integration/manual |
...
| F059 | 마지막으로 연 모음과 보기 모드 복구 | todo | `app_settings` schema exists; final review: startup restore behavior pending | integration |
| F060 | soft delete 및 명시적 library cleanup | todo | collection/icon soft delete exists; final review: explicit library cleanup command/UI pending | DB/manual |
| F061 | 원본 보기 및 개별 export 결과 보기 | todo | final review: no reveal-original or per-icon export-result UI/command found | manual |
| F062 | 200x200 JPG/PNG 대표 이미지 가져오기 | todo | final review: no standalone cover image import command/UI found | integration/UI |
